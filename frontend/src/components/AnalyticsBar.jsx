...
function Stat({ label, value, iconSrc, loading }) {
  if (loading) {
    return (
      <div
        className="h-[102px] rounded-[14px] pt-[21px] pr-[21px] pb-[1px] pl-[21px] flex flex-col gap-[8px] opacity-100 animate-pulse"
        style={{
      ...
      >
        ...
        <div className="flex items-center justify-between w-full">
          ...
          <div className="w-6 h-6 rounded-full" style={{ backgroundColor: 'var(--secondary-border)' }}></div>
        </div>
...
export default function AnalyticsBar() {
  ...
  return (
    <div
      className="grid grid-cols-3 gap-4 pr-8"
      
    >
      <Stat label="Conversations" value={totalConvos} iconSrc="/conversations.svg" loading={loadingAnalytics} />
      <Stat label="User messages" value={totalUser} iconSrc="/Group.svg" loading={loadingAnalytics} />
      {/* <Stat label="Assistant messages" value={totalAssistant} iconSrc="/u.svg" loading={loadingAnalytics} /> */}
      <Stat label="Avg time / convo" value={avgTimePerConvo} iconSrc="/g.svg" loading={loadingAnalytics} />
    </div>
  );
}
